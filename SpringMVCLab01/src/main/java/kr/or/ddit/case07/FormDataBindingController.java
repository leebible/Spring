...
public class FormDataBindingController {
	...
	/**
	 * form : form 형태의 커스텀 태그
	 * 1. form의 modelAttribute 속성으로 사용할 모델 전달
	 * 2. form:input 커스텀 태그의 path 속성이 command object의 프로퍼티로 반영됨.
	 * @return
	 */
	@GetMapping
	public String formHandler(Model model) {
		...
		return "case07/formView3"; 
	}
	
	/**
	 * post요청으로 전송된 여러개의 파라미터를 하나의 객체로 바인딩해서 받기 위한 객체 : command object ==> model attribute로 반영됨
	 * @ModelAttribute 를 이용해 command object의 모델명을 변경함
	...
	public String process(
			@Validated @ModelAttribute("adrs") AddressBookVO adrs
			, BindingResult errors
			, RedirectAttributes redirectAttributes
			) { //CommandObject 뒤에 바로 errors를 사용해야함
		...
		if(!errors.hasErrors()) {
			...
			return "redirect:/case07/handler1"; //PRG패턴 
		}
	}
